...
-- 基于这个项目：
-- https://github.com/openSUSE-Python/markdown/blob/lua55/markdown.lua
-- 更改：
-- 1. 代码块由缩进定义改为```
-- 2. 新增支持表格
...
local block_transform, span_transform, encode_code, table_cell_encode_transform
...
local function codeblocks(lines)
    local function find_codeblock(lines)
        ...
        local stop = #lines
        for i = start + 1, #lines do
            if lines[i].type == "codeblock" then
                stop = i
                break
            end
        end
        return start, stop, language
    end

    local function process_codeblock(lines, langeuge)
        local str_list = {}
        for i, line in ipairs(lines) do
            str_list[i] = encode_code(line.line)
        end
        return "<pre><code style = \"" .. langeuge .. "\">" .. table.concat(str_list, "\n") .. "\n</code></pre>"
    end

    while true do
        local start, stop, langeuage = find_codeblock(lines)
        if not start then break end
        local text = process_codeblock(splice(lines, start + 1, stop - 1), langeuage)
        local info = {
            line = text,
            type = "raw",
            html = text
...
local function tables(lines)
    local function split_line_by_pipe(text)
        local parts = {}
    ...
    local function extract_columns(text, expected_cols)
        local parts = split_line_by_pipe(text)
        -- 处理首尾竖线导致的空元素
        ...
        -- 此时parts数量应等于expected_cols，但可能因为行内空单元格而包含空字符串
        if #parts ~= expected_cols then
            -- 可选：处理错误或调整
            -- 例如如果parts数量少，可能是行缺少竖线；如果多，可能是单元格内有未转义竖线
            -- 这里简单返回nil或调整
            return nil
        end
        -- 去除每个单元格的前后空格（Markdown规范）
        local cols = {}
        for i, part in ipairs(parts) do
            -- 去除首尾空格
            cols[i] = part:match("^%s*(.-)%s*$")
        end
        return cols
    end

    local function find_table(lines)
        local separator_line_pos
        local column_count
        for i, line in ipairs(lines) do
            if line.type == "normal" then
                ...
                if text:match("^%s*|%s*$") then
                    column_count = 0
                    for _ in string.gmatch(line.text, "[^|]+") do
                        column_count = column_count + 1
                    end
                    if column_count > 0 then
                        separator_line_pos = i
                        break
                    end
                end
            end
        end
        if separator_line_pos then
            local header
            local content = {}
            if separator_line_pos > 1 then
                header = extract_columns(lines[separator_line_pos - 1].text, column_count)
            end
            for i = separator_line_pos + 1, #lines do
                local cols = extract_columns(lines[i].text, column_count)
                if cols then
                    table.insert(content, cols)
                else
                    break
                end
            end
            return separator_line_pos, content, header
        end
    end
    ...
    while (true) do
        ...
        if header then
            ...
            for i, col in ipairs(header) do
                table_inner_html_text = table_inner_html_text ..
                    "<th>" .. table_cell_encode_transform(col) .. "</th>\n"
            end
            table_inner_html_text = table_inner_html_text .. "</tr>\n</thead>\n"
        end
        table_inner_html_text = table_inner_html_text .. "<tbody>\n"
        for _, row in ipairs(content) do
            table_inner_html_text = table_inner_html_text .. "<tr>\n"
            for i, col in ipairs(row) do
                table_inner_html_text = table_inner_html_text ..
                    "<td>" .. table_cell_encode_transform(col) .. "</td>\n"
            end
            table_inner_html_text = table_inner_html_text .. "</tr>\n"
        end
        ...
        local info = {
            line = table_inner_html_text,
            type = "raw",
            html = table_inner_html_text
        }
        lines = splice(lines, start, stop, { info })
    end
    return lines
...
function table_cell_encode_transform(s)
    local sp_list = split(s, "<br>")
    for i, str in ipairs(sp_list) do
        local ss = str
        ss = ss:gsub("%&", "&amp;")
        ss = ss:gsub("<", "&lt;")
        ss = ss:gsub(">", "&gt;")
        ss = span_transform(ss)
        for k, v in pairs(escape_table) do
            ss = ss:gsub("%" .. k, v)
        end
        sp_list[i] = ss
    end
    return table.concat(sp_list, "<br>")
end
